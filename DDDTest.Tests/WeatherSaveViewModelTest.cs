...
namespace DDDTest.Tests
{
    ...
    public class WeatherSaveViewModelTest
    {
        ...
        public void 天気登録シナリオ()
        {
            ...
            var areas = new List<AreaEntity>();                 //AreaEntityの配列リストの作成
            areas.Add(new AreaEntity(1, "東京"));
            areas.Add(new AreaEntity(2, "神戸"));
            areasMock.Setup(x => x.GetData()).Returns(areas);

            var viewModelMock = new Mock<WeatherSaveViewModel>(weatherMock.Object, areasMock.Object);   //WeatherSaveViewModelをviewModelMockに代入
            ...
            var ex = AssertEx.Throws<InputException>(() => viewModel.Save());       //InputExceptionの例外をexインスタンスに代入
